...
class Config:
    ...
    weight_decay = 1e-5
    validation_split = (
        0.1  # Use 10% of training data for validation (used in final training only)
    )
    ...
    use_weighted_loss = (
        True  # *** ADDED: Flag to easily enable/disable weighted loss ***
    )
...
class FractureDataset(Dataset):
    def __init__(self, csv_file, img_dir, transform=None, indices=None):
        ...
        if indices is not None:
            ...
            if len(valid_indices) != len(indices):
                print(
                    f"Warning: Some provided indices were out of bounds for {csv_file}."
                )
        ...
        if len(self.data) == 0:
            print(
                f"Warning: No data loaded for dataset from {csv_file} with provided indices."
            )
    ...
    def __getitem__(self, idx):
        ...
        if idx >= len(self.data):
            print(
                f"Warning: Index {idx} out of bounds for dataset size {len(self.data)}."
            )
    ...
    def __call__(self, val_score):
        ...
        if self.best_score is None:
            ...
            if self.verbose and score > self.best_score:
                print(
                    f"EarlyStopping: Validation score improved ({self.best_score:.6f} --> {score:.6f}). Resetting counter."
                )
            self.best_score = score
            self.counter = 0
            return False  # Don't stop yet
...
def apply_smote(X, y):
    ...
    # Create SMOTE object
    smote = SMOTE(
        sampling_strategy=config.smote_ratio, random_state=config.random_state
    )
    ...
    with torch.no_grad():
        ...
        for batch_data in progress_bar:
            ...
            labels = labels.to(device).unsqueeze(
                1
            )  # Add channel dimension for BCE loss
            batch_size = images.size(0)
            num_processed_samples += batch_size
            ...
            )  # No detach needed in no_grad context
            preds = probs > 0.5
...
def plot_training_history(history, plot_dir, fold_num=None):
    ...
    plt.title(
        f"Accuracy (Fold {fold_num})" if fold_num else "Accuracy (Final Training)"
    )
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy")
    plt.legend()
    ...
    plt.title(
        f"F1 Score (Fold {fold_num})" if fold_num else "F1 Score (Final Training)"
    )
    plt.xlabel("Epochs")
    plt.ylabel("F1 Score")
    plt.legend()
    ...
    plot_filename = os.path.join(
        plot_dir, f"training_metrics{fold_suffix}_{timestamp}.png"
    )
...
def plot_roc_curve(labels, probs, plot_dir, dataset_name="Test"):
    ...
    plt.grid(True)
    plot_filename = os.path.join(
        plot_dir, f"{dataset_name.lower()}_roc_curve_{timestamp}.png"
    )
    plt.savefig(plot_filename)
...
def plot_precision_recall_curve(labels, probs, plot_dir, dataset_name="Test"):
    ...
    if len(np.unique(labels)) < 2:
        print(
            f"Skipping Precision-Recall curve plot for {dataset_name}: Only one class present."
        )
        return
    ...
    plt.grid(True)
    plot_filename = os.path.join(
        plot_dir, f"{dataset_name.lower()}_pr_curve_{timestamp}.png"
    )
    plt.savefig(plot_filename)
...
def plot_confusion_matrix(labels, preds, plot_dir, dataset_name="Test"):
    ...
    plot_filename = os.path.join(
        plot_dir, f"{dataset_name.lower()}_confusion_matrix_{timestamp}.png"
    )
    plt.savefig(plot_filename)
...
def plot_fold_metrics(fold_metrics, plot_dir):
    ...
    for i, metric in enumerate(metrics_to_plot):
        ...
        plt.axhline(
            y=mean_value, color="r", linestyle="-", label=f"Mean: {mean_value:.3f}"
        )
...
def train_with_kfold():
    ...
    if config.use_weighted_loss:
        ...
        if pos_samples > 0:
            current_pos_weight = torch.tensor(neg_samples / pos_samples).to(
                config.device
            )
            print(
                f"Class distribution - Negative: {neg_samples}, Positive: {pos_samples}"
            )
            print(
                f"Using positive weight of {current_pos_weight.item():.4f} for weighted loss"
            )
        else:
            print(
                "Warning: No positive samples found in the dataset. Weighted loss disabled."
            )
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_indices, y_labels)):
        ...
        if config.use_smote:
            resampled_train_indices, _ = apply_smote(
                fold_train_indices, fold_train_labels
            )
        else:
        ...
        if config.use_swa:
            ...
            swa_scheduler = SWALR(
                optimizer, swa_lr=config.swa_lr, anneal_epochs=1, anneal_strategy="cos"
            )  # Use adjusted swa_lr
        ...
        early_stopping = EarlyStopping(
            patience=config.patience, min_delta=config.min_delta, verbose=True
        )
        ...
        for epoch in range(config.num_epochs):
            ...
            val_metrics = evaluate(
                model, val_loader, criterion, config.device, desc="Validating"
            )
            ...
            # Print metrics
            print(
                f"Train | Loss: {train_metrics['loss']:.4f} | Acc: {train_metrics['accuracy']:.4f} | F1: {train_metrics['f1']:.4f} | AUC: {train_metrics['auc']:.4f}"
            )
            print(
                f"Val   | Loss: {val_metrics['loss']:.4f} | Acc: {val_metrics['accuracy']:.4f} | F1: {val_metrics['f1']:.4f} | AUC: {val_metrics['auc']:.4f}"
            )
            ...
            if val_metrics["f1"] > best_val_f1_fold:
                ...
                best_model_state_fold = {
                    k: v.cpu() for k, v in model.state_dict().items()
                }
                print(
                    f"*** New best model for Fold {fold + 1} saved with Val F1: {best_val_f1_fold:.4f} at epoch {epoch + 1} ***"
                )
            ...
            if early_stopping(val_metrics["f1"]):
                ...
                print(
                    f"Early stopping triggered at epoch {stopped_epoch} for Fold {fold + 1}"
                )
        ...
        final_model_state_to_use = (
            best_model_state_fold  # Default to best non-SWA model
        )

        if (
            config.use_swa
            and swa_model is not None
            and stopped_epoch > config.swa_start
        ):  # Check if SWA ran
            ...
            try:
                ...
                if len(train_loader) > 0:
                    torch.optim.swa_utils.update_bn(
                        train_loader, swa_model, device=config.device
                    )
                    # Evaluate SWA model
                    ...
                    print(
                        f"SWA Val | Loss: {swa_val_metrics['loss']:.4f} | Acc: {swa_val_metrics['accuracy']:.4f} | F1: {swa_val_metrics['f1']:.4f} | AUC: {swa_val_metrics['auc']:.4f}"
                    )

                    # Compare SWA F1 with the best standard model F1 for this fold
                    if swa_val_metrics["f1"] > best_val_f1_fold:
                        print(
                            f"SWA model is better for Fold {fold + 1}! Val F1: {swa_val_metrics['f1']:.4f}"
                        )
                        final_fold_metrics = swa_val_metrics
                        # Save SWA model state
                        final_model_state_to_use = {
                            k: v.cpu() for k, v in swa_model.state_dict().items()
                        }
                    else:
                        print(
                            f"Best standard model (F1: {best_val_f1_fold:.4f}) was better than SWA model (F1: {swa_val_metrics['f1']:.4f}) for Fold {fold + 1}."
                        )
                        ...
                        temp_model.load_state_dict(
                            {
                                ...
                                for k, v in best_model_state_fold.items()
                            }
                        )
                        final_fold_metrics = evaluate(
                            temp_model,
                            val_loader,
                            criterion,
                            config.device,
                            desc="Best Standard Model Re-eval",
                        )
                        del temp_model  # Clean up
                else:
                    print(
                        "Skipping SWA BN update and evaluation as train_loader is empty."
                    )
                ...
                print(
                    f"Error during SWA BN update or evaluation: {e}. Using best standard model state."
                )
                # Fallback to best standard model metrics if SWA eval fails
                temp_model = create_fracture_model().to(config.device)
                temp_model.load_state_dict(
                    {k: v.to(config.device) for k, v in best_model_state_fold.items()}
                )
                final_fold_metrics = evaluate(
                    temp_model,
                    val_loader,
            ...
            print(
                f"Using best standard model state from Fold {fold + 1} (Val F1: {best_val_f1_fold:.4f})"
            )
            temp_model = create_fracture_model().to(config.device)
            temp_model.load_state_dict(
                {k: v.to(config.device) for k, v in best_model_state_fold.items()}
            ...
            print(
                f"Warning: No best model state found for Fold {fold + 1}. Skipping metrics."
            )
    ...
    if valid_fold_metrics:
        ...
        original_fold_num = (
            valid_fold_indices[best_fold_idx_overall] + 1
        )  # Get the original fold number
        ...
        print(
            f"Saved best k-fold model state from Fold {original_fold_num} to {config.best_kfold_model_path}"
        )

        return (
            valid_fold_metrics,
            best_state_dict,
        )  # Return metrics and the best state dict
    else:
...
def train_final_model():
    ...
    for epoch in range(config.num_epochs):
        ...
        print(
            f"Valid | Loss: {val_metrics['loss']:.4f} | Acc: {val_metrics['accuracy']:.4f} | F1: {val_metrics['f1']:.4f} | AUC: {val_metrics['auc']:.4f}"
        )
        ...
        if val_metrics["f1"] > best_val_f1:
            ...
            print(
                f"*** New best model for Final Training saved with Val F1: {best_val_f1:.4f} at epoch {epoch + 1} ***"
            )

        if early_stopping(val_metrics["f1"]):
            stopped_epoch = epoch + 1
            print(
                f"Early stopping triggered at epoch {stopped_epoch} for Final Training"
            )
            break
        ...
        try:
            if len(train_loader) > 0:
                ...
                if swa_val_metrics["f1"] > best_val_f1:
                    print(
                        f"SWA model is better! Final Val F1: {swa_val_metrics['f1']:.4f}"
                    )
                    ...
                    print(
                        f"Best standard model (F1: {best_val_f1:.4f}) was better than SWA model (F1: {swa_val_metrics['f1']:.4f})."
                    )
            else:
                print("Skipping SWA BN update and evaluation as train_loader is empty.")
        except Exception as e:
            print(
                f"Error during SWA BN update or evaluation in final training: {e}. Using best standard model state."
            )
    ...
    plot_training_history(
        history, config.plot_dir, fold_num=None
    )  # Pass None for final training plot
...
def test_model(model_state_dict):
    ...
    try:
        ...
        test_model.load_state_dict(
            {k: v.to(config.device) for k, v in model_state_dict.items()}
        )
    ...
    test_metrics = evaluate(
        test_model, test_loader, criterion, config.device, desc="Testing"
    )
    ...
    if len(test_metrics["labels"]) > 0:
        ...
        try:
            ...
            print(
                f"Could not generate classification report: {e}"
            )  # Handle case where only one class is predicted/present
